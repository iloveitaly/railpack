...
| `deployOutputs` | Generated filters for the final image                         |
...
| `inputs`       | Final image layers                                             |
| `aptPackages`  | List of Apt packages to install in the final image                      |

### Deploy Inputs and Step Outputs

Railpack generates deploy inputs from providers and configured steps. A step's
`deployOutputs` controls which parts of that step are included in these
generated inputs:

- Omit `deployOutputs` to include the step's working directory.
- Set `deployOutputs` to `[]` to include nothing from the step.
- Provide filters to include specific files or directories.

When `deploy.inputs` is provided, it replaces every generated input, including
all step `deployOutputs`:
...
```json
{
  ...
  "deploy": {
    // Only "other" is deployed; the generated "dist" output is replaced.
    "inputs": [{ "step": "build", "include": ["other"] }]
  }
}
```

Add `...` to retain generated inputs at that position:
...
An explicit empty list, `deploy.inputs: []`, deploys no generated inputs.
...
Railpack allows comments in `railpack.json`, but VS Code treats files with a
...
The [`railpack.json` schema](https://schema.railpack.com) should be recognized
